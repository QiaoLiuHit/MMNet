% ----------------------------------------------------------------------------------------------------------------
function net = make_branch_vggnet(varargin)
...
% ----------------------------------------------------------------------------------------------------------------
    opts.exemplarSize = [127 127]; %[127 127]
    opts.instanceSize = [255 255]; %[255 255]
    opts.last_layer = 'conv5_3';
    ...
    % added lr changed
    net.meta.trainOpts.learningRate = [0.01*ones(1,10) 0.001*ones(1,10) 0.0001*ones(1,10) 0.00001*ones(1,10) 0.000001*ones(1,10)] ;
...
% --------------------------------------------------------------------
function net = add_block_conv_only(net, opts, id, h, w, in, out, stride, pad)
% --------------------------------------------------------------------
info = vl_simplenn_display(net) ;
fc = (h == info.dataSize(1,end) && w == info.dataSize(2,end)) ;
if fc
...
convOpts = {'CudnnWorkspaceLimit', opts.cudnnWorkspaceLimit} ;
net.layers{end+1} = struct('type', 'conv', 'name', sprintf('%s%s', name, id), ...
                           'weights', {{init_weight(opts, h, w, in, out, 'single'), ones(out, 1, 'single')*opts.initBias}}, ...
                           'stride', stride, ...
                           'pad', pad, ...
                           'learningRate', [1 2], ...
                           'dilate', 1, ...
...
% --------------------------------------------------------------------
    ...
    [ideal_exemplar, ~] = ideal_size(net, opts.exemplarSize);%ideal_exemplar=127*127
    [ideal_instance, ~] = ideal_size(net, opts.instanceSize); %ideal_instance=255-2*8
...
function net = vgg_16(net, opts)
% --------------------------------------------------------------------

net.layers = {} ;
net = add_block(net, opts, '1_1', 3, 3, 3, 64, 1, 0) ;

net = add_block(net, opts, '1_2', 3, 3, 64, 64, 1, 0) ;
...
net = add_block(net, opts, '2_1', 3, 3, 64, 128, 1, 0) ;

net = add_block(net, opts, '2_2', 3, 3, 128, 128, 1, 0) ;
...
net = add_block(net, opts, '3_1', 3, 3, 128, 256, 1, 0) ;

net = add_block(net, opts, '3_2', 3, 3, 256, 256, 1, 0) ;

net = add_block(net, opts, '3_3', 3, 3, 256, 256, 1, 0) ;
...
net = add_block(net, opts, '4_1', 3, 3, 256, 512, 1, 0) ;

net = add_block(net, opts, '4_2', 3, 3, 512, 512, 1, 0) ;

net = add_block(net, opts, '4_3', 3, 3, 512, 512, 1, 0) ;
...
net = add_block(net, opts, '5_1', 3, 3, 512, 512, 1, 0) ;

net = add_block(net, opts, '5_2', 3, 3, 512, 512, 1, 0) ;

net = add_block_conv_only(net, opts, '5_3', 3, 3, 512, 64, 1, 0) ;
